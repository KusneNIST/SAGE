...
class SAGE_ND(Joint):
    ...
    def run(self):
        ...
        gpc_var_bounds = jnp.asarray(self.gpc_variance_bounds, dtype=jnp.float64).copy()
        gpc_ls_bounds = jnp.asarray(self.gpc_lengthscale_bounds, dtype=jnp.float64).copy()
        gpr_var_bounds = jnp.asarray(self.gpr_variance_bounds, dtype=jnp.float64).copy()
        gpr_ls_bounds = jnp.asarray(self.gpr_lengthscale_bounds, dtype=jnp.float64).copy()
        gpr_noise_bounds = jnp.asarray(self.gpr_noise_bounds, dtype=jnp.float64).copy()
        gpr_bias_bounds = jnp.asarray(self.gpr_noise_bounds, dtype=jnp.float64).copy()
        Xpred = jnp.asarray(self.locations_prediction, dtype=jnp.float64).copy()
        num_regions = self.num_phase_regions
        num_proc = self.num_chains

        idx_Xnew_exclude_xs, idx_Xnew_match_xs, idx_xs_match_Xnew = identify_X_overlap_with_x(Xpred, xs)
        ...
        predict_fn_structure = lambda samples: predict_structure(
                samples, self.predict_SAGE_ND_PM, Xpred, xs, ys, num_regions=num_regions
            )
        predict_fn_sage_1core = lambda samples: predict_sage(
                samples, self.predict_SAGE_ND, Xpred, xs, ys, xf, yf, num_regions=num_regions, idx_Xnew_exclude_xs=idx_Xnew_exclude_xs, idx_Xnew_match_xs=idx_Xnew_match_xs, idx_xs_match_Xnew=idx_xs_match_Xnew, idx_xf_exclude_xs=idx_xf_exclude_xs
            )
        ...
        gpc_SVI_probs_, _ = predict_fn_structure(mle_st)
        
        gpc_latent_ = mle_st['gpc_latent_0']
        for i in range(1,num_regions):
            gpc_latent_ = jnp.vstack((gpc_latent_,mle_st['gpc_latent_' + str(i)]))
            
        preds_fp = None
        preds_st = None
        
        init_params = {'gpc_var': mle_st['gpc_var'],'gpc_lengthscale': mle_st['gpc_lengthscale'], 'gpc_bias': mle_st['gpc_bias']}
        for i in range(num_regions):
            init_params['gpc_latent_'+str(i)] = mle_st['gpc_latent_'+str(i)]
        ...
        predict_fn_sage = jax.pmap(
            lambda samples: predict_sage(
                ...
                gpr_ls_bounds=gpr_ls_bounds, gpr_bias_bounds=gpr_bias_bounds, gpr_noise_bounds=gpr_noise_bounds, idx_Xnew_exclude_xs=idx_Xnew_exclude_xs, idx_Xnew_match_xs=idx_Xnew_match_xs, idx_xs_match_Xnew=idx_xs_match_Xnew, idx_xf_exclude_xs=idx_xf_exclude_xs
            ), axis_name = 0
        )
        ...
        preds_stacked_[labels[0]] = preds_stacked[labels[0]].reshape(-1,Xpred.shape[0],num_regions)
        preds_stacked_[labels[3]] = preds_stacked[labels[3]].reshape(-1,Xpred.shape[0],num_regions)
        ...
        self.predictions = {'phase_region_labels_mean_estimate':phase_region_labels_mean_estimate,
                      ...
                      'functional_property_mean':functional_properties_mean,
                      'functional_property_std':functional_properties_std,
                      'phase_region_labels_SVI':gpc_SVI_probs_,
                       }
    ...
    def predict_SAGE_ND(self, Xnew, xs, ys, xf, yf, num_regions, gpc_var_bounds, gpc_ls_bounds,
            gpr_var_bounds, gpr_ls_bounds, gpr_bias_bounds, gpr_noise_bounds, idx_Xnew_exclude_xs=None, idx_Xnew_match_xs=None, idx_xs_match_Xnew=None, idx_xf_exclude_xs=None):
        ...
        for j in range(num_regions):
            f = compute_f_matern52_jax(gpc_var,
                      gpc_lengthscale,
                      gpc_bias,
                      gpc_latent[j][idx_x_exclude_overlap_with_xs], x_[idx_x_exclude_overlap_with_xs,:], jitter=self.jitter)
            
            gpc_train_latent = gpc_train_latent.at[:,j].set(f)
            gpc_noise = self.jitter
            mean, cov, _ = gpr_forward_matern52_jax(gpc_var, gpc_lengthscale ,x_[idx_x_exclude_overlap_with_xs,:],f, Xnew_no_xs, gpc_noise, include_noise=False, jitter=self.jitter)
            fhat = ndist.MultivariateNormal(mean, cov + jnp.eye(N_Xnew_no_xs) * self.jitter).sample(subkey)
    ...
    def predict_SAGE_ND_PM(self, Xnew, xs, ys, num_regions, eps=1E-6, gpc_var_bounds=jnp.asarray([0.1,10.]), gpc_ls_bounds=jnp.asarray([.5,10.])):
...
def identify_X_overlap_with_x(X, x):
    m_Xo_x, idx_X_match_x, idx_x_match_X = compare_inputs_jax(X, x)
    idx_X_exclude_x = np.setdiff1d(np.arange(X.shape[0]), idx_X_match_x )
    return idx_X_exclude_x, idx_X_match_x, idx_x_match_X