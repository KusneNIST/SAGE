...
class SAGE_ND(Joint):
    ...
    def predict_SAGE_ND(self, Xnew, xs, ys, xf, yf, num_regions, gpc_var_bounds=jnp.asarray([0.1,10.]), gpc_ls_bounds=jnp.asarray([.5,10.]), \
        ...
        for j in range(num_regions):
            ...
            fhat = ndist.MultivariateNormal(mean, cov + jnp.eye(cov.shape[0]) * 1E-4).sample(subkey)
            gpc_new_latent = gpc_new_latent.at[:,j].set(fhat)

            # temp_mean = jnp.sum(jnp.isnan(mean.flatten()))
            # jax.debug.print("Pred, NaN mean: {t}", t = mean)
            # temp_cov = jnp.sum(jnp.isnan(cov.flatten()))
            # jax.debug.print("Pred, NaN cov: {t}", t = cov)
